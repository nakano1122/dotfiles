...
計画立案・実装着手の前に、以下のスキルで要件の曖昧さを炙り出す:

| スキル | 用途 | タイミング |
|--------|------|-----------|
| `/grill-me` | 計画の曖昧さを質問で検証 | 計画フェーズ開始時 |
| `/grill-with-docs` | CONTEXT.md を参照して用語・ドメイン整合性を検証 | ドメインモデルに関わる機能実装前 |

**推奨フロー:**
```
1. 要件を受け取る
2. /grill-me で曖昧な点を質問（ユーザーと対話）
3. /grill-with-docs で用語・ドメインの整合性を確認
4. 計画立案へ進む
```

プロジェクトに `CONTEXT.md` がある場合、`/grill-with-docs` が用語集・ドメインモデルを参照して検証を行う。

## プロジェクト立ち上げワークフロー

```
0. 計画検証（grill）
   → /grill-me, /grill-with-docs で要件の曖昧さを解消
1. 要件整理
   → 機能要件・非機能要件の整理、制約の確認
2. 技術選定
